...
APPS = {
    ...
    "reading/sherlock-redheaded-4": (reads("redheaded4"),
                                     dict(app="redheaded4", shake="#card", lift=None,
                                          dragon=["m1"])),
    # The Engineer's Thumb, the next story, a block at a time: pages 87 to 94,
    # then 95 to 103. More will follow as they are set.
...
SKIN_OPEN = "/* MC-SKIN-CSS: generated by build_theme.py, do not hand-edit */"
SKIN_SHUT = "/* /MC-SKIN-CSS */"


def skin_css(src):
    """Give a site page the wardrobe rules from mc.css.

    Themes, purses and the rank chip have to reach the hub, the trophy room and
    the store, none of which get the whole stylesheet. Same shape as mute_css:
    delimited at the source, bounded here, so a re-theme replaces rather than
    appends.
    """
    css = (THEME / "mc.css").read_text(encoding="utf-8")
    m = re.search(r"/\* MC-SKIN-CSS-START \*/(.*?)/\* MC-SKIN-CSS-END \*/", css, re.S)
    if not m:
        return src
    block = "%s\n%s\n%s\n" % (SKIN_OPEN, m.group(1).strip(), SKIN_SHUT)
    old = re.search(re.escape(SKIN_OPEN) + r".*?" + re.escape(SKIN_SHUT) + r"\n?", src, re.S)
    if old:
        return src[:old.start()] + block + src[old.end():]
    return src.replace("</style>", block + "</style>", 1)
...
def store():
    ...
    so it is also the only page where MC.buy is called."""
    return page(STUDY / "store" / "index.html",
                '<script src="../_build/mc.js"></script>', "store")
